...
/*
 * DB연동하는 프로젝트에는 필수적으로 구현해야 할 method가 있다.
...
public interface BookService {
	...
	// 도서 정보 데이터를 1개 전달받아서 DB insert를 하는 기능 수행
	// 실제 도서 정보 데이터는 ISBN, 도서명, 출판사코드, 저자코드
	// 		발행일, 가격, 페이지의 데이터가 포함된다.
	// 도서 정보를 VO 객체에 담아 전달받아 DB에 insert 기능 수행
	...
	// PK 한개를 전달받아 조회하는 method는 0 또는 1개의 값만 return한다.
	// 데이터가 없을 때는 null을 있을때는 실제 데이터가 담긴 DTO가 return 된다.
