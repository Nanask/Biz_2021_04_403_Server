...
public class BookRentServiceImplV1 implements BookRentService{
	...
	protected Connection dbConn;
	    ...
	    * ecception 처리 방법 2가지
	    * 1. try-catch를 사용하기
	    * 		exception이 발생할 코드를 감싸서 "직접" exception을 처리하기
	    * 
	    * 2. 떠넘기기(던지기)
	    * 		method throws를 추가하여 호출한 method에게 exception을 되돌려 보내기
	    * 		현재 method에서 직접 exception을 처리하는 코드가 없어서 코드가 다소 간소화 된다.
	    * 
...
//			}
	...
	public BookRentDTO findById(long seq) {
		 ...
		 * 
		 * Prepare... 를 사용할때는 조건문의 변수가 포함될 위치에 ? 기호를 사용한다.
		 * WHERE 도서명 = ? 과 같이 사용한다.
		 *  
		 */
