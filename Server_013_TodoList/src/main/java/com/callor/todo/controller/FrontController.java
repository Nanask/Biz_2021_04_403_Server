...
public class FrontController extends HttpServlet {
	...
	// 한번 실행되어서
	// 여러가지 변수 등을 초기화 하는 코드 , method
	// java util Map import
	
	// 인터페이스인 todoCommand를 변수로 만든다?
	// 왜 todocommand를 쓰는가?
	protected Map<String, TodoCommand> commands;

	// FrontController가 최초 호출될 때 한번 실행되어서 여러가지 변수 등을 초기화 하는 코드
	@Override
	public void init(ServletConfig config) throws ServletException {

		commands = new HashMap<String, TodoCommand>();

		// 대리자 등록? subpath가 올 경우 여기서 진행하도록 한다?
		// 만약 http://localhost : 8080/todo/rootPath/ 로 요청이 오면 HomeCommandImplV1 객체를 사용하여
		// 요청을 처리하기 위한 준비
		
		commands.put("/", new HomeCommandImplV1()); // 경로 저장, 실행하지도 않고 이동하지도 않지만 
		// "/"키 값 , impl이 value

		// 만약 http://localhost : 8080/todo/insert/로 요청이 오면
		// HomeCommandImplV1 객체를 사용하여 요청을 처리하기 위한 준비
		commands.put("/insert", new TodoCommandImplV1()); 
		commands.put("/expire", new TodoCommandImplV1());
	}

	// doGet(), doPost()로 분리하여 요청을 처리하던 방식
	...
	protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		// Web에서 요청한 path 가져오기
		String urlPath = req.getRequestURI(); // urI의 전체 path를 가져온다.
		String path = urlPath.substring(req.getContextPath().length()); // .getContextPath() 이게 루트를 가지고 있음 = 여기서는 todo
		// substring 길이를 자르는 것
		// rootpath를 뺀 나머지! 그러니까 여기서는 todo / 여기꺼!!!!! todo 뒤에!!!!!

		System.out.println(path);

		// req 된 URI 중에서 실제 subPath 부분을 사용하여 처리할 객체를 Map으로 부터 추출
		// TodoCommand command = new TodoCommandImplV1();
		TodoCommand subCommand = commands.get(path); // 저장해놓은 커멘드를 불러옴
		if (subCommand != null) {

			// 각 Command 객체의 execute() method에게 실제 요청을 처리하도록 위임하기
